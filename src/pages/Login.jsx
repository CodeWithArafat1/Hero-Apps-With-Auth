...
// Google এবং GitHub আইকনের জন্য SVG কোড
const GoogleIcon = () => (
  <svg
...
const Login = () => {
  ...
  const handleGitHubLogin = async () => {
    try {
      ...
      let currentUser = createuser.user;

      if (currentUser && !currentUser.email && currentUser.providerData) {
        const gitprovider = currentUser.providerData.find(
          (p) => p.providerId === "github.com"
        );

        if (gitprovider && gitprovider.email) {
          currentUser = { ...currentUser, email: gitprovider.email };
        }
      }
